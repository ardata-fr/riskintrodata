...
get_wahis_erf <- function(
  ...
  cli_abort_if_not(
    ...
    "Value {.arg {disease}} does not exists for column {.arg disease} in WAHIS dataset." =
      disease %in% unique(wahis_erf$disease),
    "Value {.arg {species}} does not exists for column {.arg species} in WAHIS dataset." =
      species %in% unique(wahis_erf$species),
    "{.arg animal_category} must be either {.val Domestic} or {.val Wild}." =
      animal_category %in% c("Domestic", "Wild")
  )
